...
TARGET=""
RUNS=2
OUTPUT_FILE="$OUTPUT_DIR/bench_kernel_build_default.txt"
CLEAN=false
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --output FILE     结果输出文件（默认 output/bench_kernel_build_default.txt）"
...
echo "内核编译耗时（默认调度器）" | tee -a "$OUTPUT_FILE"
