...
grep -q "#define WAKEUP_PREEMPT_GRAN_NS" src/eevdf.bpf.c && echo "   ✓ WAKEUP_PREEMPT_GRAN_NS 已定义" || echo "   ✗ 缺少 WAKEUP_PREEMPT_GRAN_NS"
grep -q "#define WAKEUP_KICK_MIN_INTERVAL_NS" src/eevdf.bpf.c && echo "   ✓ WAKEUP_KICK_MIN_INTERVAL_NS 已定义" || echo "   ✗ 缺少 WAKEUP_KICK_MIN_INTERVAL_NS"
...
echo ""
echo "7. 验证唤醒抢占检查（enqueue 路径）..."
grep -q "static __always_inline void eevdf_kick_preempt_if_needed" src/eevdf.bpf.c && echo "   ✓ 唤醒抢占检查函数存在" || echo "   ✗ 缺少唤醒抢占检查函数"
grep -q "eevdf_kick_preempt_if_needed\\(p," src/eevdf.bpf.c && echo "   ✓ enqueue 调用了唤醒抢占检查" || echo "   ✗ enqueue 未调用唤醒抢占检查"
grep -q "cpu_last_kick_ns" src/eevdf.bpf.c && echo "   ✓ 每 CPU kick 限频 map 存在" || echo "   ✗ 缺少 cpu_last_kick_ns 限频 map"
grep -q "SCX_KICK_PREEMPT" src/eevdf.bpf.c && echo "   ✓ 使用 SCX_KICK_PREEMPT 触发重调度" || echo "   ✗ 未找到 SCX_KICK_PREEMPT 使用"
grep -q "SCX_KICK_IDLE" src/eevdf.bpf.c && echo "   ✓ 使用 SCX_KICK_IDLE 做 idle kick" || echo "   ✗ 未找到 SCX_KICK_IDLE 使用"

echo ""
echo "8. 验证时间片策略（当前为固定 3ms）..."
grep -q "return 3000000ULL;" src/eevdf.bpf.c && echo "   ✓ eevdf_calculate_slice: 固定 3000000ns" || echo "   ⚠ 未找到固定 3ms（可能已改为动态算法）"

echo ""
echo "9. 检查系统 sched_ext 支持..."
...
echo "  ✓ 唤醒抢占检查：eligible 且更早 vd 时 kick 触发重调度（带粒度/限频）"
...
echo "  sudo bash scripts/test.sh --io-only --duration 15"
